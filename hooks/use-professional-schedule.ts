...
export function useProfessionalSchedule() {
  ...
  const checkIn = async (appointmentId: string, data: { location: { lat: number; lng: number }; notes?: string }) => {
    setIsLoading(true)
    setError(null)
    try {
  ...
  const requestReschedule = async (appointmentId: string, data: { reason: string; suggestedDates: string[] }) => {
    setIsLoading(true)
    setError(null)
    try {
  ...
  const updateAvailability = async (data: { date: string; slots: { start: string; end: string }[] }[]) => {
    setIsLoading(true)
    setError(null)
    try {
