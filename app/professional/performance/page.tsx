  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Track your metrics and reviews</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{metrics?.totalReviews || 0}</div>
          ...
          <CardContent>
            ...
            <div className="flex items-center">
              {[1, 2, 3, 4, 5].map((star) => (
                <Star
                  key={star}
                  className={`h-4 w-4 ${star <= (metrics?.averageRating || 0) ? "text-yellow-400 fill-yellow-400" : "text-gray-300"}`}
                />
              ))}
            </div>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{metrics?.weeklyReviews || 0}</div>
        ...
        <TabsContent value="reviews" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Client feedback about your services</CardDescription>
            </CardHeader>
                ...
                reviews.map((review) => (
                  <div key={review.id} className="p-4 border rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      <div>
                        <h4 className="font-semibold">{review.serviceType}</h4>
                        <p className="text-sm text-muted-foreground">Client: {review.customerName}</p>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <Button variant="outline" size="sm" onClick={() => openResponseDialog(review)}>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm">{Math.round(((metrics?.averageRating || 0) / 5) * 100)}%</span>
                </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium">Reviews This Month</span>
                  <span className="text-sm">{metrics?.monthlyReviews || 0}</span>
                </div>
              ...
              <div className="flex items-center gap-2 mb-2">
                <div className="flex">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <Star
                      key={star}
                      className={`h-4 w-4 ${star <= (selectedReview?.rating || 0) ? "text-yellow-400 fill-yellow-400" : "text-gray-300"}`}
                    />
                  ))}
                </div>
